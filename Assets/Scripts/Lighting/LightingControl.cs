...
public class LightingControl : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            string key = lightingBrightness.month.ToString() + lightingBrightness.hour.ToString();
    ...
    private void EventHandler_AdvanceGameHourEvent(int gameYear, Month gameMonth, int gameDay, string gameDayOfWeek, int gameHour, int gameMinute, int gameSecond)
    {
        SetLightingIntensity(gameMonth, gameHour, true);
    }
    ...
    {
        Month gameMonth = TimeManager.Instance.GetGameMonth();
        int gameHour = TimeManager.Instance.GetGameTime().Hours;

        // Set light intensity immediately without fading in
        SetLightingIntensity(gameMonth, gameHour, false);
    }

    /// <summary>
    /// Set the light intensity based on season and game hour
    /// </summary>
    private void SetLightingIntensity(Month gameMonth, int gameHour, bool fadein)
    {
        int i = 0;
        ...
        while (i <= 23)
        {
            ...
            string key = gameMonth.ToString() + (gameHour).ToString();
